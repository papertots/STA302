...
We notice a few signs that our model violates linear regression assumptions. The residual vs. fitted graph shows a pattern of heteroscedasticity, possibly because the variance of the error term is not constant. This makes sense in the context of our data, since players are guaranteed a minimum salary regardless of their performance, which shows up as the clustering of data points in the bottom left part of the graph. On the other hand, factors such as fame affect top range players more than lowered ranged players, and we don't have sufficient data to account for that in our model. 

We also notice that the expected value of the residuals is not 0 across the range. The model slightly underestimates player salary on the top and bottom range, and we explained above why this is the case. The same applies for the violation of *normality of error* assumption shown from the normal-QQ plot and standardized residuals histogram, where the data appears right-skewed. Otherwise, the standardized residuals is approximately normal from the -2.5 to 1 standard deviation range.

For all the violations above, we have considered applying a log transform on our response variable, but that's beyond the scope of the first part of the project. 

<!--
\newpage

## Additional (Irrelevant) Plots
-->
