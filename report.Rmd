...
This means that, along with more obvious statistics like scoring, playmaking, and defense, factors such as team role and fame are also taken into account.
...
Linear regression is an appropriate statistical tool for this analysis because we are interested in estimating the average change in salary associated with unit changes in predictor variables (such as points per game and assists). The method allows us to quantify the effect of each variable while holding others constant. Additionally, categorical predictors such as a player's main position can be incorporated, enabling the model to account for role-based differences in compensation.
...
In the model, we use Salary (USD) as the response variable to investigate which aspects of player performance and role most correlate to player valuation. Since `Salary` is a continuous and non-negative variable that reflects NBA compensation, it is suitable for linear regression.
...
- Personal Fouls Per Game (PFoul): Reflective of a player’s defensive discipline and quality (Figure \@ref(fig:vars).C)
...
- Main Position (Pos1: [PG: point guard, SG: shooting guard, SF: small forward, PForward: power forward, C: center]): A categorical variable that reflects role on court.
...
The final row of Figure \@ref(fig:scatter-matrix) shows that the response variable (Salary) is roughly linear in the 4 continuous predictor variables. We theorize that Model \@ref(eq:lm) would be a good linear approximation.
\begin{equation}
\begin{split}
Salary &= \beta_0 + \beta_1\ PTS + \beta_2\ PFoul + \beta_3\ AST + \beta_4\ STL\\
...
The scatter plot of response vs. fitted values (Figure \@ref(fig:diagnostic-2), left) also suggests that our model is not reliable for predicting salary in terms of the predictors we defined.
...
In particular, we notice from the estimated model parameters (as seen in Table \@ref(tab:prelim-model)) that the predictor, assists (AST), contributes to the average salary by about \$1.2 million ($\hat{\beta_3} \approx `r nba_model[["coefficients"]]["AST"] |> round(-3)`$) for each unit increase while holding all other predictors constant. The predictor personal fouls (PFoul) has an opposite effect ($\hat{\beta_2} \approx `r nba_model[["coefficients"]]["PF"] |> round(-3)`$).
...
We chose the log-transform for the Salary response variable to fix the violation of the constant variance assumption,
and a mix of log and square/cube root transformations for the predictor variables to fix the violation of linearity assumption. For log-transform of predictor variables, a small constant (the square of the first quartile divided by the third quartile of each predictor) is added to avoid taking the log of 0 [@muldoon_log-0_2018].
...
\end{split}
\end{equation}
Its associated diagnostic plots are included below in Figure \@ref(fig:diagnostic-transformed). While violations of constant variance, linearity, and normality of errors are still present in the plots, they are significantly improved compared to the original model, and are approximately satisfied to the degree to reasonably allow us to use partial F-tests to eliminate predictors. It is reasonable since F-test is considered *robust* against violations of normality of error and constant variance specifically [@berger_experimental_2018].
...
reduced_anova <- anova(mod.reduced, mod.transformed)
```

We propose the following reduced model
\begin{equation}
\begin{split}
...
where the p-value for the t-test for each individual predictor's coefficient is high.
A partial F-test with Model \@ref(eq:reduced) as $H_0$,
and the transformed full Model \@ref(eq:transformed) above as $H_1$ gives us a p-value of `r reduced_anova[["Pr(>F)"]][2] |> round(3)`,
which is large enough to indicate that the reduced model should be used.
...
and Figure \@ref(fig:diagnostic-additional) and \@ref(fig:diagnostic-additional-cont) show that
there are no major regression assumption violations for the updated Model \@ref(eq:additional),
...
The spread of points around 0 is also approximately the same and therefore approximately satisfies the constant variation of error assumption.
In Figure \@ref(fig:diagnostic-additional) (right), the distribution of standardized residuals is slightly left-skewed, but it's close enough to normality that most of our analysis (F-tests and confidence intervals) still hold by robustness of ANOVA F-test [@berger_experimental_2018] and central limit theorem (with $n=$ `r nrow(mod.additional$model)`), respectively.
\begin{equation}
\begin{split}
...
A partial F test gives us a very significant p-value of $2.2\times 10^{-16}$, while each estimated coefficient has a variance inflation factor $<5$ (see Table \@ref(tab:summary-table)).
Therefore, we will use this as our final model for analysis.
...
Figure \@ref(fig:leverage).B only shows 1 data point whose $|r_i| > 4$ ($n= `r toString(n)`$, large enough to use 4 as cutoff), so we only have 1 outlier in our current model.

From Figure \@ref(fig:leverage).A, there are leverage points in the dataset, where the blue line is the $h = \frac{2(p+1)}{n}$ cutoff. Since we only have 1 outlier, all but one of our leverage points are good, so we will check influential measures using Cook's distance, DFFITS, and DFBETAS measurements next.
...
Manually inspecting these points does not reveal any invalid data values for all predictors before or after transformations, so they can't be removed based on the invalid data criterion. Because our model contains already transformed variables, we attempt to use interaction terms to eliminate or reduce these observations, with no effect. The plots used to reach this conclusion is omitted from this report for brevity, but the code to generate them can be found inside the R Markdown file next to this section.
...
# repeat the following to check outliers, leverage and influential points
model_infl <- get_infl_measures(mod.treated)
graph_obs(model_infl)
graph_inf(model_infl)
```

```{r robust-regression, fig.cap="OLS vs IWSL fits for continuous predictors. 't' in axis label denotes that the variable has been transformed. Note the minimal difference between the slopes of the different regression estimation methods."}
...
Given the large size of the dataset ($n= `r nrow(nba)`$) and relatively few number of estimated predictors ($p=$ `r length(mod.reduced$coefficients)`), the cutoffs for DFFITS, DFBETAS, and the alternate cutoff for Cook's Distance provided in lecture are too small and sensitive to determine useful influential points.
...
since a one-unit increase in the transformed predictors corresponds to an expected $\hat{\beta_i}$ unit increase in $log($`Salary`$)$,
holding all other predictors constant.
In other words, it's also associated with an expected increase in `Salary` by a factor of $e^{\hat{\beta_i}}$,
i.e., an increase of $(e^{\hat{\beta_i}} - 1)100\%$ [@ford_interpreting_2018].
End-points of the (95%) confidence interval are transformed and interpreted similarly, that we are 95% confident that the true expected average percentage increase lies within the interval.
...
Our model uses SF/PF (Forwards) as the reference level, but it would be inappropriate to interpret the intercept estimate directly, since player age is a predictor and should not take a value of 0.
Compared to the forwards (SF and PForward), center (C) players are expected to earn $\phantom{}`r b_hat["tPos1Center"]`$ more on average, point guards (PG) earn $\phantom{}`r b_hat["tPos1Point Guard"] |> gsub("-", "", x = _)`$ less,
...
# backwards elimination using our model as the full model and justify not eliminating any variables
elim <- setup_measures()
elim <- get_measures(elim, mod.additional, 4)
elim <- get_measures(elim,
                     lm(tSalary ~ tAST + tFvot + Age + tPos1,
                        data = nba), 5)
elim <- get_measures(elim,
                     lm(tSalary ~ tPTS + tFvot + Age + tPos1,
                        data = nba), 6)
elim <- get_measures(elim,
                     lm(tSalary ~ tPTS + tAST + Age + tPos1,
                        data = nba), 7)
elim <- get_measures(elim,
                     lm(tSalary ~ tPTS + tAST + tFvot + tPos1,
                        data = nba), 8)
elim <- get_measures(elim,
                     lm(tSalary ~ tPTS + tAST + tFvot + Age,
                        data = nba), 9)
list(measures, elim) |> kable(digits = 3, booktabs = TRUE, format = "latex", escape = FALSE,
                  caption = "Left: Previous model performance metrics.\nRight: Backwards elimination related performance metrics.",
                  col.names = c("Model", "AIC", "BIC", "$R^2$", "$R^2_{adj}$")) |>
  kable_styling(latex_options = "hold_position")
```

In Table \@ref(tab:model-measures) (Left), we provide several key metrics, including Akaike Information Criterion (AIC), Bayesian Information Criterion (BIC), $R^2$, and $R^2_{adj}$ for our final model as well as each of our previous models.
Model 1 served as our initial model, Model 2 applied transformations to address violations of linear assumptions, Model 3 retained much of the characteristics of Model 2 without redundant predictors, and Model 4 is our final and strongest performing model.

When constructing Model 4, our goal was to achieve the best performance across all metrics, i.e., minimize AIC and BIC while maximizing $R^2_{adj}$.
The $R^2$ values indicate how well each model explains the variability in player salaries, and we compare model explanatory performance by using $R^2_{adj}$.
Model 4 outperforms the other 3 models with this metric, with $R^2_{adj}$ = 0.517.
The AIC and BIC values in Table 3 indicate that Model 4 also offers clear advantages in model fit when compared to previous model iterations.
It has the lowest AIC at 2951.74 when compared to all other models.
The BIC values, which more strongly penalize model complexity than AIC, also identify Model 4 as the best performing, with the lowest BIC at 2997.97. So Model 4 best explains the variation of Salary while remaining the least complex (i.e., the most efficient) model.

Table \@ref(tab:model-measures) (Right) justifies the lack of variable removal from our final model using performance metrics.
Models 5-9 are Model 4 with each of the 5 predictors removed in order.
Similar to previous analysis, Model 4 is the best model in terms of $R^2_{adj}$ and AIC, and the second best in terms of BIC.
Therefore, Model 4 remains our final model.
...
with centers generally earning more ($\phantom{}`r b_hat["tPos1Center"]`$) and guards earning less ($`r b_hat["tPos1Shooting Guard"] |> gsub("-", "", x = _)`$ for shooting guards and $`r b_hat["tPos1Point Guard"] |> gsub("-", "", x = _)`$ for point guards) compared to forwards, our reference group.

On the other hand, personal fouls (PFoul) and steals (STL) are predictors that are highly correlated with others,
...
In brief, according to our model, the primary way for a player to improve their salary would be to improve their on-court performance in terms of their scoring and playmaking ability.
Players aiming to be more popular may see an increase in their salary as well, but the extent of the improvement may be minor.
Additionally, salary differences reflect positional value, as centers historically command higher pay for their defensive impact,
while forwards, valued for their versatility, typically see more balanced compensation.
Point guards were also paid significantly less than forwards as seen in our results.
However, it’s important to note that this reflects the period covered by our dataset.
Since 2019, the landscape has shifted, with point guards surpassing centers as the league’s highest-paid position [@curcic_nba_2024].
This evolution reflects the modern game's guard-driven, three-point-oriented offensive style.
...
Using the R-squared measure, we see that our model explains 52% of the variation in $log(\text{Salary})$, which leaves 48% unexplained.
...
Contributions of each group member is detailed as follows:

Yizhe Zhao

- R code used to analyze data and generate the plots and tables, as well as the R markdown formatting (including citations) for outputting the final report pdf from Rmd
- Preliminary model results (Section \@ref(preliminary-results)) and Model Selection (Section \@ref(model-selection)) of the report

Shi Wang

- Formatting, rewriting, citations, and general proofreading of the report
- Assisted in model selection and assumption verification
- Co-wrote Section \@ref(model-performance-assessment)

Valentyn Lytvyniuk

- Proofreading, organizing in-person meeting, co-writing introduction and data description, collaborating on the data set and response/predictor variable selection, collaborating on transformation selection, interpretation of transformation, and co-writing conclusion section

Cyre Beroncal

- Wrote initial drafts of introduction and data description sections, proposed research question
- Calculated back-transformations (Section \@ref(model-summary))
- Wrote interpretations of all coefficients in the context of the problem (Section \@ref(coefficient-interpretations))
- Wrote initial draft of Recommendations for Future Research (Section \@ref(recommendations-for-future-research)), providing suggestions in contract interaction and NBA contextualization.
...
- Co-wrote Section \@ref(model-performance-assessment). Integrated and helped write both sections of the conclusion (\@ref(conclusions-and-key-findings), \@ref(recommendations-for-future-research))
