...
nba.hist.Salary <- ggplot(nba) +
  geom_histogram(aes(Salary, y = after_stat(density)), 
                 bins = diff(range(nba$Salary) / (2 * IQR(nba$Salary) / n ^ (1/3)))
                 |> round()) +
...
nba.hist.PTS <- ggplot(nba) +
  geom_histogram(aes(PTS, y = after_stat(density)), 
                 bins = diff(range(nba$PTS) / (2 * IQR(nba$PTS) / n ^ (1/3)))
                 |> round()) +
...
nba.hist.PF <- ggplot(nba) +
  geom_histogram(aes(PF, y = after_stat(density)),
                 bins = diff(range(nba$PF) / (2 * IQR(nba$PF) / n ^ (1/3)))
                 |> round()) +
  geom_density(aes(PF)) +
...
nba.hist.AST <- ggplot(nba) +
  geom_histogram(aes(AST, y = after_stat(density)),
                 bins = diff(range(nba$AST) / (2 * IQR(nba$AST) / n ^ (1/3)))
                 |> round()) +
...
nba.hist.STL <- ggplot(nba) +
  geom_histogram(aes(STL, y = after_stat(density)), 
                 bins = diff(range(nba$STL) / (2 * IQR(nba$STL) / n ^ (1/3)))
                 |> round()) +
...
graph_summaries_2 <- function(){
  p3 <- ggplot(nba_model) +
    geom_point(aes(x = .fitted, y = nba$Salary)) +
