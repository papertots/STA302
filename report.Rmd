...
```{r leverage, fig.cap="Plot of leverage values against index."}
graph_obs <- function(model) {
  p <- length(model$coefficients)
  n <- nrow(model$model)
  h_bar <- (p + 1) / n
  df <- influence.measures(model)$infmat |> as.data.frame()
  df <- df |>
    mutate(index = as.numeric(row.names(df))) |>
    mutate(sresid = rstandard(model))
  p1 <- ggplot(df) +
    geom_linerange(aes(x = index, ymin = 0, ymax = hat)) +
    geom_hline(aes(yintercept= 2 * h_bar), colour = "blue") +
