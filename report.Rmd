...
and a mix of log and square/cube root transformations for the predictor variables to fix violation of linearity assumption.

The model **[TODO: INSERT MODEL HERE]** and its associated diagnostic plots are included below. While violations of non-constant variance and linearity, and normality of errors are still present in the plots, they are significantly improved compared to the original model, and are approximately satisfied to the degree to reasonably allow us to use partial F-tests to eliminate predictors, if we use a conservative $\alpha$ value.

## Partial F-test

We propose reduced model **[TODO: INESRT MODEL HERE]**, which removes the transformed STL and PF predictors, as well as collapsing SF and PF categorical levels into the same level. The choices in these predictors are inspired by a summary of the model from R, which we will omit here. A partial F-test with the above model as $H_0$, and the transformed full model above as $H_1$ gives us a p-value of 0.267, which is large enough to indicates that the reduced model should be used, even with linear regression assumptions only approximately satisfied. 
