...
var_label(nba) <- list(
  tPTS = "sqrt(Points per game)",
  tAST = "log(Assists per game)",
  tSTL = "log(Steals per game)",
  tPF = "log(Personal fouls per game)"
)
```
...
&+ \beta_4 Age + \beta_5\ I(SF \lor PF) + \beta_6\ I(PG) + \beta_7\ I(SG) + \varepsilon
(\#eq:additional)
\end{split}
\end{equation}
...
```{r summary-table}
tbl_regression(mod.reduced, intercept = T)
