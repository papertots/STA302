...
&+ \beta_3\ I(SF \lor PF) + \beta_4\ I(PG) + \beta_5\ I(SG) + \varepsilon
(\#eq:reduced)
\end{split}
\end{equation}
...
From Figure \@ref(fig:leverage).A, there are leverage points in the dataset, where the blue line is the $h = \frac{2(p+1)}{n}$ cutoff. Since we do not have any outliers, all leverage points are good leverage points, so we will check influential measures using Cook's distance, DFFITS, and DFBETAS measurements next.
...
infl_df <- nba |>
  mutate(over.cook.4.n = mod.inflences$over.cook.4.n,
         over.dffits = mod.inflences$over.dffits,
         over.dfbetas = mod.inflences$over.dfbetas,
         over.all = mod.inflences$over.all,) |>
  slice(which(over.all))
graph_inf(mod.inflences)
```

Figure \@ref(fig:influential) shows that the dataset has no influential points using Cook's cutoff ($F_{0.5,\ p+1,\ n-p-1} =$ `r toString(round(cook_cutoff, 3))`),
but has `r infl_df |> filter(over.cook.4.n) |> nrow() |> toString()` influential points using a different cutoff ($\frac{4}{n}=$ `r toString(round(4/n, 3))`).
Alternatively, the DFFITS measure shows `r infl_df |> filter(over.dffits) |> nrow() |> toString()` influential points
(with cutoff $2\sqrt{\frac{p+1}{n}}=$ `r toString(round(2 * sqrt((p+1)/n), 3))`),
while the DFBETAS measure shows `r infl_df |> filter(over.dfbetas) |> nrow() |> toString()` influential points
(with cutoff $\frac{2}{\sqrt{n}}=$ `r toString(round(2 / sqrt(n), 3))`).

Manually inspecting these points do not reveal any invalid data values for all predictors before or after transformations, so they can't be removed based on invalid data criteria. Because our model contains already transformed variables, we attempt to use interaction terms to eliminate these observations, with no effect. The plots used to reach this conclusion is omitted from this report for brevity, but the code to generate them can be found inside the R Markdown file next to this section.

```{r influential-treatment, include=FALSE}
mod.reduced.treated1 <- lm(update(mod.reduced, . ~ . + tPos1:tPTS))
mod.reduced.treated2 <- lm(update(mod.reduced, . ~ . + tPos1:tAST))
mod.reduced.treated3 <- lm(update(mod.reduced, . ~ . + tPTS:tAST))

# We repeat the following for each of the 3 interaction we defined
mod.inflences.treated <- get_infl_measures(mod.reduced.treated1)
graph_obs(mod.inflences.treated)
graph_inf(mod.inflences.treated)
```

```{r robust-regression, fig.cap="OLS vs IWSL fits for continuous predictors. Note the minimal difference between the slopes."}
mod.robust <- rlm(formula(mod.reduced), data = nba)
plot_pred <- function(pred) {
  result <- ggplot(nba, aes(x = eval(as.name(pred)), y = tSalary)) +
    geom_point() +
    labs(x = pred) +
...
plot_grid(plot_pred("tPTS"), plot_pred("tAST"))
```
...
In summary, we opt to leave the influential points and our model as is, since our model has no outliers, very few leverage points, and no influential points (using Cook's cutoff). Given the large size of the dataset ($n=$ `r nrow(nba) |> toString()`) and relatively few number of estimated predictors ($p=$ `r length(mod.reduced$coefficients) |> toString()`), the cutoffs for DFFITS, DFBETAS, and alternate cutoff for Cook's Distance provided in lecture are too small and sensitive to determine useful influential points. Robust regression fits also do not provide any benefits to us, so we will not use robust regression for our analysis either.

```{r sensitivity-analysis, include=F}
sub.nba <- nba |>
