...
- Main Position (Pos1: [PG: point guard, SG: shooting guard, SF: small forward, PF: power forward, C: center]): A categorical variable that reflects role on court
...
\begin{equation}
Salary = \beta_0 + \beta_1\ PTS + \beta_2\ PFoul + \beta_3\ AST + \beta_4\ STL + \beta_5\ I(PForward) + \beta_6\ I(PG) + \beta_7\ I(SF) + \beta_8\ I(SG) + \varepsilon
(\#eq:lm)
\end{equation}
...
Manually inspecting these points do not reveal any invalid data values for all predictors before or after transformations, so they can't be removed based on the invalid data criterion. Because our model contains already transformed variables, we attempt to use interaction terms to eliminate or reduce these observations, with no effect. The plots used to reach this conclusion is omitted from this report for brevity, but the code to generate them can be found inside the R Markdown file next to this section.
...
```{r robust-regression, fig.cap="OLS vs IWSL fits for continuous predictors. 't' in axis label represents denotes that the variable has been transformed. Note the minimal difference between the slopes of the different regression estimation methods."}
...
Similar to previous analysis, Model 4 is the best model in terms of $R^2_{adj}$ and AIC, and second best in terms of BIC.
Therefore, Model 4 remains as our final model.
