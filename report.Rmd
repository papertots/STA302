...
```{r var-additional, fig.cap="Distribution of the additional predictors."}
...
```{r diagnostic-additional, fig.cap="Diagnostic of the new fitted model after incorporating additional predictors."}
...
Figure \@ref(fig:var-additional) shows their distribution, and Figure \@ref(fig:diagnostic-additional) shows that regression assumptions are all still approximately satisfied for the updated Model \@ref(eq:additional), similar to what we saw earlier in Figure \@ref(fig:diagnostic-transformed).
Specifically, while the blue mean line in Figure \@ref(fig:diagnostic-additional) (left) is not completely straight, it's close enough to a constant of zero to approximately satisfy the assumption of linearity.
...
p_round_fn <- function(num) {
  result <- signif(num, 3) |> as.character()
  return(result)
}
round_fn <- function(num) {
  result <- round(num, 3) |> as.character()
  return(result)
}
regression_tbl <- tbl_regression(
  ...
  mod.additional, intercept = T, pvalue_fun = p_round_fn, estimate_fun = round_fn) |>
  modify_caption("Fitted regression model summary") |>
  modify_header(label = "**Predictor Variables**", estimate = "**Beta estimate**")
  #modify_footnote(update = everything() ~ "Predictors values are per game values.") |>
# show_header_names(regression_tbl)
regression_tbl <- regression_tbl |>
  as_kable_extra(booktabs = TRUE, escape = FALSE) |>
  kable_styling(latex_options = "hold_position")
regression_tbl
```
...
measures_tbl <- kable(measures, digits = 3, format = "latex",
