...
```{r summary-table, results="markup", fig.cap="Fitted regression model summary."}
...
i.e. an increase of $(e^{\hat{\beta_i}} - 1)100\%$.
End-points of the (95%) confidence interval is transformed and interpreted similarly, that we are 95% confident that the true expected percentage increase lies within the interval.
...
The coefficient for Age suggesting that for each additional year of age, salary increases by approximately $\phantom{}`r b_hat["Age"]`$. [**We could move this part to section 6 to talk about how we can improve our analysis with restricted cubic splines.**] However, this linear model cannot capture the full parabolic nature of career earnings in the NBA. Typically, players receive large raises after their rookie-scale contracts (usually after 3–4 years in the league), leading to a steep rise in earnings through their prime (roughly ages 25–30). After that point, declining athletic performance, injuries, and team roster priorities may reduce earning power, especially for non-star players.
